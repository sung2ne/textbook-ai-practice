...
# CIFAR-10 데이터 로드
transform = transforms.Compose([transforms.ToTensor()])
cifar_dataset = datasets.CIFAR10('./data', train=True, download=True, transform=transform)
cifar_loader = DataLoader(cifar_dataset, batch_size=128, shuffle=True)

# 학습률별 손실 변화 비교 (각 50배치씩)
results = {}
...
# CIFAR-10 소규모 데이터셋 (과적합을 빠르게 관찰하기 위해 2,000장만 사용)
...
train_set = Subset(full_dataset, range(1500))
val_set = Subset(full_dataset, range(1500, 2000))

train_loader = DataLoader(train_set, batch_size=64, shuffle=True)
val_loader = DataLoader(val_set, batch_size=64, shuffle=False)

# 과적합되기 쉬운 모델 (Dropout 없음)
class SimpleCNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.features = nn.Sequential(
...
for epoch in range(20):
    ...
    train_accs.append(correct / len(train_set) * 100)
    ...
    val_accs.append(correct / len(val_set) * 100)
...
plt.tight_layout()
plt.show()
print(f"최종 학습 정확도: {train_accs[-1]:.1f}% / 검증 정확도: {val_accs[-1]:.1f}%")
